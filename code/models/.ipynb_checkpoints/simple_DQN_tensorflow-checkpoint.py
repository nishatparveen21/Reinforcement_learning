...
class DeepQNetwork:
    ...
    def learn(self):
        ...
        # See if the two nets still stay the same - evaluation on the same state = s
        # print(" ------------ AFTER update -------------")
        # for pos in range(20):
        #     for vel in range(5):
        #         state = [[pos, vel]]
        ...
        if cost > 1000:
            print(f"Big loss - state ={cost}")
        print(f"Big loss at ={batch_memory[:, :self.n_features]}")
        # print("loss ={global_step_update}")
